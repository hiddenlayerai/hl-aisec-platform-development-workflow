...
echo "Switching action references from @v2 to @$BRANCH_NAME..."

# Find all YAML files and replace @v2 with branch reference
find .github/workflows -name "*.yml" -type f | while read -r file; do
    if grep -q "hl-aisec-reference-actions.*@v2" "$file"; then
        echo "Updating: $file"
        # Use sed to replace @v2 with @branch
        sed -i.bak "s|hl-aisec-reference-actions/\([^@]*\)@v2|hl-aisec-reference-actions/\1@$BRANCH_NAME|g" "$file"
        rm "$file.bak"
    fi
done
...
echo "Remember to switch back to @v2 after creating the release tag" 